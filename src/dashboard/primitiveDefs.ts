...
export const PRIMITIVES: PrimitiveDef[] = [
...

  {
    ...
    blurb: 'What the Assistant may do WITHOUT asking. One form — a composer-footer control, the same idiom as Mode and Reasoning level — because a mandate is a setting, not a message. The control does not EXPLAIN the four rungs, it SHOWS them: the rungs are named in a list, and underneath the full permission MATRIX is drawn as one — the actions a legal assistant actually takes, ordered by consequence (lire → rédiger → modifier → enregistrer dans la GED → envoyer à un tiers), as rows; the four rungs as columns; every cell marked seul / demande / jamais. All four levels are comparable at a glance, so choosing one is picking a column you have already read — no meaning hidden behind a hover, a selection, or a reopen. Two things are deliberately encoded rather than stated: anything leaving the cabinet still asks even at full autonomy — that is how delegation works in a firm, an associate can draft and file freely and still not write to opposing counsel unsupervised — and an active `wall` CLAMPS every outward action to jamais, so you watch the cloison bite instead of reading that it applies. `level` is the mandate; `wall` is state the agent discovers, not something you set. Whose rights it borrows is one short row, not a paragraph: C13\'s role axis answers who can pay, this answers who can see.',
