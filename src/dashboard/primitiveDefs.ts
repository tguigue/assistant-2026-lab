...
export const PRIMITIVES: PrimitiveDef[] = [
...

  {
    ...
    blurb: 'What the Assistant may do WITHOUT asking. One form — a composer-footer control, the same idiom as Mode and Reasoning level — because a mandate is a setting, not a message. The four rungs use the shape every assistant has already taught its users — the model-picker idiom: one line per rung, one line of consequence beneath it, check on the current one, all four readable while the menu is open. (Earlier drafts — a live permission column, then a full permission matrix — were more expressive and read as foreign; familiarity IS the design constraint for a composer control.) The discipline that survives: the consequence line states OPERATIVE FACTS — what the rung lit, rédige, modifie, enregistre or envoie, seul or on your accord — never a slogan like « Je prépare, vous appliquez », which leaves a lawyer guessing whether the courrier gets sent. The last rung\'s line carries the rule that never moves: anything leaving the cabinet still asks even at full autonomy — that is how delegation works in a firm, an associate can draft and file freely and still not write to opposing counsel unsupervised. An active `wall` suspends outward action regardless of the rung, announced as a band the agent DISCOVERS — `level` is the mandate; `wall` is state you don\'t set. Whose rights it borrows is one short row, not a paragraph: C13\'s role axis answers who can pay, this answers who can see.',
